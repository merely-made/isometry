...
| [2026-08-23_runtime_profile_plan.md](2026-08-23_runtime_profile_plan.md) | **Active behind the protocol and host prerequisites.** Product-owned `isometry-runtime` conducts an event-driven, zero-step Conatus projection only after ordered Isometry events apply; its optional Quint resident body-position consumer proves sparse GPU publication without promoting a shared frame or lease. Desktop wiring waits for protocol H2 and the Genet host migration; shared conductor/source/frame contracts wait for a second game. |
