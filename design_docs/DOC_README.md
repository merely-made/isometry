...
| [2026-08-23_runtime_profile_plan.md](2026-08-23_runtime_profile_plan.md) | **Active behind the protocol and host prerequisites.** Product-owned `isometry-runtime` conducts accepted zero-step Conatus changes into an optional Quint resident body plane and configurable fixed-isometric GPU tenant, then composes its same-device texture through Netrender. Desktop wiring waits for protocol H2 and the Genet host migration; shared conductor, source, frame, lease, and tenant contracts wait for a second game. |
